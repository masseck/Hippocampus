...
def main() -> None:
    ...
    print("=" * 60)
    print("postprocess_spikes_isi_group.py — path configuration")
    ...
    print(f"Postprocessing {len(manifests)} folder(s) for group ISI plots...")
    ...
    out_root = os.path.join(
        ...
        "Postprocessing", "Spike_ISI_Group",
    )
    ensure_dir(out_root)
    ...
    for (region_name, kind), d in grouped.items():
        ...
        plot_group_isi(
            ...
            out_png=os.path.join(
                region_kind_root,
                f"ISI_group_{region_name}_{kind}_{window_suffix}.png",
            ),
            out_svg=os.path.join(
                region_kind_root,
                f"ISI_group_{region_name}_{kind}_{window_suffix}.svg",
            ),
            title=(
        ...
        print(
            f"\u2713 Saved group ISI plot: {region_name}/{kind}/{window_suffix} "
            f"(n={n_animals})"
        )
